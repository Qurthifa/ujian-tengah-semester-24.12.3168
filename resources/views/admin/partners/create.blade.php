...
<div class="max-w-2xl mx-auto bg-white p-8 md:p-10 rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-300">
    <div class="mb-8">
        <h2 class="text-2xl font-black text-slate-800 tracking-tight">Tambah Partner Baru</h2>
        <p class="text-sm text-slate-400 mt-1">Lengkapi informasi di bawah untuk mendaftarkan mitra baru.</p>
    </div>
    ...
    <form action="{{ route('admin.partners.store') }}" method="POST" class="space-y-6">
        ...
        <div class="space-y-2">
            ...
            <input type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: Universitas Amikom" 
        ...
        <div class="space-y-2">
            ...
            <input type="url" id="logo_url_input" name="logo_url" value="{{ old('logo_url') }}" placeholder="Contoh: https://images.unsplash.com/photo-12345" 
                   class="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 text-sm focus:outline-none focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all duration-200">
            <span class="text-xs text-slate-400 font-medium flex items-center gap-1 mt-1.5 pl-1">
                ...
                Salin dan tempel langsung tautan link gambar logo partner dari internet ke sini.
            </span>
            @error('logo_url') <p class="text-rose-500 text-xs mt-1.5 font-semibold pl-1">{{ $message }}</p> @enderror
        </div>

        <div class="space-y-2 pt-2">
            <label class="block text-sm font-bold text-slate-700 tracking-wide">Pratinjau Gambar Logo</label>
            <div class="w-32 h-32 bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl flex items-center justify-center p-3 transition-all duration-300 group hover:border-indigo-500/50">
                <div class="w-full h-full bg-white border border-slate-100 rounded-xl flex items-center justify-center shadow-sm overflow-hidden p-2">
                    <img id="logo_preview_img" 
                         src="{{ old('logo_url') ? old('logo_url') : 'https://placehold.co/200x200?text=No+Image' }}" 
                         alt="Preview Logo" 
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        urlInput.addEventListener('input', function () {
            ...
            if (currentUrl) {
                ...
                previewImg.src = 'https://placehold.co/200x200?text=No+Image';
            }
        });

        previewImg.addEventListener('error', function() {
            if (urlInput.value.trim() !== '') {
                previewImg.src = 'https://placehold.co/200x200?text=Link+Error';
            }
        });
    });
</script>
