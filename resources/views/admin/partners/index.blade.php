...
@section('content')
<div class="mb-8 pl-1">
    <h2 class="text-2xl font-black text-slate-800 tracking-tight">Manajemen Partner ><</h2>
    <p class="text-sm text-slate-400 mt-1">cari, tambah, atau perbarui seluruh Supporter Event.</p>
</div>
...
<div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-4 rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
    ...
    <form action="{{ route('admin.partners.index') }}" method="GET" class="w-full sm:w-auto">
        {{-- Search Input Modern --}}
        ...
        <table class="w-full text-left border-collapse">
            <thead class="bg-slate-50/70 border-b border-slate-100 text-slate-400 uppercase text-[10px] font-black tracking-widest">
                <tr>
                    ...
                    <th class="px-6 py-4">Nama Partner / Alamat Tautan</th>
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="hover:bg-slate-50/50 transition-colors duration-200 group">
                    ...
                    {{-- Kolom Preview Logo --}}
                    <td class="px-6 py-4">
                        <div class="w-20 h-14 bg-slate-50 border border-slate-100 p-2 rounded-xl flex items-center justify-center overflow-hidden shadow-inner group-hover:bg-white transition-colors duration-200">
                    ...
                    {{-- Nama Partner --}}
                    <td class="px-6 py-5">
                        <p class="font-black text-slate-800 text-base tracking-tight group-hover:text-indigo-600 transition-colors duration-200">{{ $partner->name }}</p>
                        <p class="text-slate-400 text-xs font-medium mt-1 truncate max-w-md bg-slate-50 border border-slate-100 px-2 py-0.5 rounded-md inline-block" title="{{ $partner->logo_url }}">
                            {{ $partner->logo_url }}
                        </p>
                    </td>
                ...
                <tr>
                    <td colspan="4" class="px-8 py-12 text-center text-slate-400 font-medium text-sm">
                        Belum ada data partner resmi yang tersedia.
                    </td>
                </tr>
                @endforelse
            </tbody>
