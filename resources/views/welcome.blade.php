   ...
   <div class="mb-8 flex gap-4 justify-center">
    ...
    @foreach($categories as $cat)
        <a href="/?category={{ $cat->slug }}" 
           class="px-4 py-2 rounded shadow-sm transition font-medium 
           {{ request('category') == $cat->slug ? 'bg-gray-300 text-black font-bold' : 'bg-indigo-100 hover:bg-indigo-200 text-indigo-700' }}">
            {{ $cat->name }}
        </a>
    @endforeach
</div>
...
<section class="max-w-7xl mx-auto px-6 py-16 border-t border-slate-100">
    <div class="text-center mb-12">
        <h2 class="text-2xl font-extrabold text-slate-800 tracking-tight">Supported By Partners</h2>
    ...
    <div class="flex flex-wrap justify-center gap-6 max-w-5xl mx-auto">
        ...
        <div class="w-40 bg-white border border-slate-100 p-4 rounded-2xl shadow-sm flex flex-col items-center justify-center py-6 hover:shadow-md hover:border-indigo-500/20 transition-all duration-300 group shrink-0">
            ...
            <div class="h-20 w-full flex items-center justify-center">
                <img src="{{ \Illuminate\Support\Str::startsWith($partner->logo_url, 'http') ? $partner->logo_url : asset($partner->logo_url) }}"
                    alt="{{ $partner->name }}"
                    title="{{ $partner->name }}"
                    class="max-h-20 max-w-full object-contain transition duration-300 group-hover:scale-105">
            </div>
            <p class="text-xs font-bold text-slate-600 tracking-wide text-center mt-3 uppercase group-hover:text-indigo-600 transition truncate w-full">
                {{ $partner->name }}
            </p>
        </div>
        @empty
